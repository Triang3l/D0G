...
int CTexture::ComputeActualSize( bool bIgnorePicmip, IVTFTexture *pVTFTexture )
{
	...
	//
	int iHwWidth, iHwHeight, iHwDepth;
#ifdef SHADERAPI_GLES2
	if ( IsCubeMap() )
	{
		iHwWidth = iHwHeight = HardwareConfig()->MaxCubemapSize();
		iHwDepth = 1;
	}
	else if ( m_nActualDepth > 1 )
	{
		iHwWidth = iHwHeight = iHwDepth = HardwareConfig()->MaxTextureDepth();
	}
	else
	{
		iHwWidth = HardwareConfig()->MaxTextureWidth();
		iHwHeight = HardwareConfig()->MaxTextureHeight();
		iHwDepth = 1;
	}
#else
	iHwWidth = HardwareConfig()->MaxTextureWidth();
	iHwHeight = HardwareConfig()->MaxTextureHeight();
	iHwDepth = HardwareConfig()->MaxTextureDepth();
#endif
