...
#else
	static int CompareHelper( const T *lhs, const T *rhs, void *context )
#endif
	{
...
extern "C"
{
	void V_qsort_r(void *a, size_t n, size_t es, int (*cmp)(const void *, const void *, void *), void *thunk);
}
#endif
...
#else
	typedef int (__cdecl *QSortCompareFunc_t)(const void *, const void *, void *context);
#endif
	if ( this->Count() > 1 )
	{
		QSortContext_t ctx;
		...
		V_qsort_r( this->Base(), this->Count(), sizeof(T), (QSortCompareFunc_t)&CUtlSortVector<T, LessFunc>::CompareHelper, &ctx );
#else
		qsort_r( this->Base(), this->Count(), sizeof(T), (QSortCompareFunc_t)&CUtlSortVector<T, LessFunc>::CompareHelper, &ctx );
#endif
	}
}
