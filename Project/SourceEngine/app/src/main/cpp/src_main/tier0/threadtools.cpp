...
#elif _LINUX
	pthread_t tid;
	pthread_create(&tid, NULL, (void *(*)(void *))ThreadProcConvert, new ThreadProcInfo_t( pfnThread, pParam ) );
	if ( pID )
		*pID = (ThreadId_t)tid;
...
bool CThreadSyncObject::Wait( uint32 dwTimeout )
{
   ...
   return ( VCRHook_WaitForSingleObject( m_hSyncObject, dwTimeout ) == WAIT_OBJECT_0 );
	...
	do
	{   
	    ret = pthread_cond_timedwait( &m_Condition, &m_Mutex, (struct timespec *)&tm );
	} 
	while( ret == EINTR );
...
#elif _LINUX
		snprintf( m_szName, sizeof(m_szName) - 1, "Thread(0x%p/0x%lx)", this, m_threadId );
#endif
