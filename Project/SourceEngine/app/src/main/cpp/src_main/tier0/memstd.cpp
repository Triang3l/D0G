	...
	if ( !pResult )
	{
		...
		for (;;)
		{
			...
			else
			{
				...
				{
					if ( pCommitLimit + POOL_COMMIT_SIZE <= m_pAllocLimit )
					{
						if ( !VirtualAlloc( pCommitLimit, POOL_COMMIT_SIZE, VA_COMMIT_FLAGS, PAGE_READWRITE ) )
						{
							Assert( 0 );
							return NULL;
						}

						m_pCommitLimit = pCommitLimit + POOL_COMMIT_SIZE;
					}
					else
					{
						return NULL;
